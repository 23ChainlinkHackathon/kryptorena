...
contract Kryptorena is ERC1155, Ownable, ERC1155Supply {
    ...
    function _endBattle(
        ...
        generateNewAttDef(_battle);

        return _battle;
    }

    function generateNewAttDef(Battle memory _battle) internal {
        address playerAddress = msg.sender;
        Player memory player = getPlayer(playerAddress);
        require(player.inBattle, "Player is not in a battle");

        uint256 playerIndex = playerInfo[playerAddress];
